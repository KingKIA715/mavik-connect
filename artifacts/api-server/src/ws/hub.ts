...
/**
 * DM connections use a separate map, keyed by thread id, so thread ids and
 * group ids (both plain integers) never collide with each other.
 */
...
export function registerThreadConnection(
  ...
  broadcastThreadPresence(threadId);
  return conn;
}

export function unregisterThreadConnection(threadId: number, conn: Connection): void {
...
function broadcastThreadPresence(threadId: number): void {
  const set = threadConnections.get(threadId);
  if (!set) return;
  const userIds = Array.from(new Set(Array.from(set).map((c) => c.userId)));
  broadcastToThread(threadId, { type: "presence", userIds });
}
...
export function sendToThreadUser(
  threadId: number,
  userId: string,
  payload: unknown,
): void {
